...
export class BlockchainService {
  ...
  // 🔹 Read recent events (last 2000 blocks)
  async getRecentEvents(): Promise<{ events: EventItemResponse[]; timestamp: string }> {
    try {
      const latestBlock = await this.client.getBlockNumber();
      const fromBlock = latestBlock - BigInt(2000);
      ...
      const mappedEvents: EventItemResponse[] = events.map((event: any) => ({
